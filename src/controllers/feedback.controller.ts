...
export class FeedbackController {
  ...
  createFeedback = async (
    ...
    try {
      ...
      if (req.body.category === "general") {
        const existing = await this.feedbackRepository.findByUserId(
          req.user._id.toString()
        );
        if (existing.some((fb) => fb.category === "general")) {
          return res.status(409).json({
            success: false,
            message: "You have already submitted site feedback.",
          });
        }
      }
  ...
  getUserFeedback = async (
    ...
    try {
      ...
      const feedback = await this.feedbackRepository.findByUserId(
        req.user._id.toString()
      );
