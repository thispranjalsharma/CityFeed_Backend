...
export class EventManagerController {
  ...
  async getManagedEventTicketBookings(req: Request & { user?: { _id: string, role: string } }, res: Response) {
    try {
      ...
      if (!event.managerId || event.managerId.toString() !== managerId.toString()) {
        return res.status(403).json({ 
          success: false, 
          message: 'Forbidden: You can only access ticket bookings for events you manage' 
        });
      }
