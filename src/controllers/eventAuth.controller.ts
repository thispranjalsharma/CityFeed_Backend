...
export class EventAuthController {
  constructor(
    @inject("EventAuthService")
    private readonly eventAuthService: EventAuthService
  ) {}
  ...
  register = async (req: Request, res: Response) => {
    try {
      ...
      const { organizer, token } =
        await this.eventAuthService.registerEventOrganizer({
          name,
          email,
          password,
          phone,
  ...
  verifyEmail = async (req: Request, res: Response) => {
    try {
      ...
      const organizer = await this.eventAuthService.verifyEmail(token);
      res.status(200).json({
        success: true,
        data: { organizer },
  ...
  updateProfile = async (req: Request, res: Response) => {
    try {
      ...
      let profile;
      if (user.role === "event_organizer") {
        profile = await EventOrganizer.findByIdAndUpdate(user._id, updates, {
          new: true,
        });
      } else if (user.role === "event_staff") {
        profile = await EventStaff.findByIdAndUpdate(user._id, updates, {
          new: true,
        });
      } else {
        return res
  ...
  deleteEventOrganizerProfile = async (
    ...
    try {
      ...
      if (!organizer) {
        return res
          .status(404)
          .json({ success: false, message: "Event organizer not found." });
      }
      organizer.isDeleted = true;
      await organizer.save();
      return res.status(200).json({
        success: true,
        message: "Event organizer profile marked as deleted.",
      });
    } catch (err: any) {
  ...
  deleteEventManagerProfile = async (
    req: Request & { user?: { _id: string } },
    res: Response
  ) => {
    try {
      ...
      return res.status(200).json({
        success: true,
        message: "Event manager profile marked as deleted.",
      });
    } catch (err: any) {
      return res.status(400).json({ success: false, message: err.message });
    }
  ...
  deleteEventStaffProfile = async (
    ...
    try {
      ...
      return res.status(200).json({
        success: true,
        message: "Event staff profile marked as deleted.",
      });
    } catch (err: any) {
      return res.status(400).json({ success: false, message: err.message });
    }
  ...
  getMyEventManagers = async (req: Request, res: Response) => {
    try {
      const user = (req as any).user;
      if (!user || user.role !== "event_organizer") {
        return res.status(403).json({
          success: false,
          message: "Only event organizers can access their event managers.",
        });
      }
  ...
  getMyEventStaff = async (req: Request, res: Response) => {
    try {
      const user = (req as any).user;
      if (!user || user.role !== "event_organizer") {
        return res.status(403).json({
          success: false,
          message: "Only event organizers can access their event staff.",
        });
      }
      const managers = await EventManager.find({ createdBy: user._id });
      const managerIds = managers.map((m: any) => m._id);
      const staff = await EventStaff.find({
        $or: [{ createdBy: user._id }, { createdBy: { $in: managerIds } }],
      });
  ...
  // Optional: EventManager and EventStaff profile getters
  getEventOrganizerProfile = async (
    req: Request & { user?: { _id: string } },
    res: Response
  ) => {
    try {
