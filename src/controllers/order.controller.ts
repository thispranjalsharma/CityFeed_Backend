...
import {
  ...
  decrementTicketTierSoldCount,
  decrementEventTotalSoldCount,
...
export class OrderController {
    ...
    try {
      ...
      if (!eventId || !Array.isArray(tickets) || tickets.length === 0) {
        return res.status(400).json({
          success: false,
          message: "Event ID and at least one ticket are required.",
        });
      }
      if (!user) {
        return res
      ...
      if (tiers.length === 0) {
        ...
        if (typeof event.ticketPrice !== "number" || event.ticketPrice <= 0) {
          return res.status(400).json({
            success: false,
            message:
              "No ticket tiers exist and event.ticketPrice is not set or invalid.",
          });
        }
        ...
        if (totalQuantity < 1) {
          return res.status(400).json({
            success: false,
            message: "At least one ticket must be purchased.",
          });
        }
        ...
        const orderTickets: OrderTicketDTO[] = [
          {
            ticketTierId: null,
      ...
      if (tiers.length !== tickets.length) {
        return res.status(404).json({
          success: false,
          message: "One or more ticket tiers not found for this event.",
        });
      }
      ...
      for (const t of tickets) {
        const tier = tiers.find(
          (tt) => tt._id?.toString() === t.ticketTierId.toString()
        );
        if (!tier) continue;
        ...
        const activeSessionsForTier = Array.from(
          ...
          (session) =>
            session.tierId === t.ticketTierId && session.eventId === eventId
        );
        ...
        const actuallyAvailable =
          tier.quantity - (tier.soldCount || 0) - reservedQuantity;
      ...
      if (allTiers.length > 0) {
        ...
        totalAvailableSeats =
          event.venue.capacity - (event.totalSoldCount || 0) - reservedQuantity;
      }
  ...
  async payWithCoins(req: Request & { user?: any }, res: Response) {
    try {
        ...
        if (hasTicketTiers) {
          ...
          for (const ticket of order.tickets) {
            if (ticket.ticketTierId) {
              const tier = tiers?.ticketTiers.find(
              ...
              if (tier) {
                ...
                if (available <= 0) {
                  return res.status(400).json({
                    success: false,
                    message: `Tickets are no longer available for ${tier.name}. Please try a different ticket type or contact support.`,
                  });
                }
                if (ticket.quantity > available) {
                  return res.status(400).json({
                    success: false,
                    message: `Only ${available} tickets available for ${tier.name}, but you ordered ${ticket.quantity}. Please reduce quantity or try a different ticket type.`,
                  });
                }
              }
            }
          ...
          if (event && event.venue && event.venue.capacity) {
            ...
            if (totalOrderedQuantity > available) {
              return res.status(400).json({
                success: false,
                message: `Only ${available} tickets available, but you ordered ${totalOrderedQuantity}. Please reduce quantity or contact support.`,
              });
            }
          }
        }
      ...
      const { DineInSessionRepository } = await import(
        "../repositories/dineInSession.repository"
      );
      const { OutletRepository } = await import(
        "../repositories/outlet.repository"
      );
      ...
      const finalAmount = Math.max(
        0,
        Math.round(totalAmount - (discountResult?.discountAmount || 0))
      );
      ...
      if (allTiers?.ticketTiers.length > 0) {
        availableSeats = allTiers.ticketTiers.reduce(
          (sum, tier) => sum + ((tier.quantity || 0) - (tier.soldCount || 0)),
          0
        );
      } else {
  ...
  async requestOrderCancellation(req: Request & { user?: any }, res: Response) {
    try {
      ...
      for (const ticket of order.tickets) {
        if (ticket.ticketTierId) {
          hasTiers = true;
          await decrementTicketTierSoldCount(
            ticket.ticketTierId.toString(),
            ticket.quantity,
            order.event.toString()
          );
      ...
      if (!hasTiers) {
        await decrementEventTotalSoldCount(
          order.event.toString(),
          order.tickets.reduce((sum, t) => sum + t.quantity, 0)
        );
      }
      ...
      if (user) {
        ...
        if (order.tickets && Array.isArray(order.tickets)) {
          refundAmount = order.tickets.reduce(
            (sum, t) => sum + t.priceAtPurchase * t.quantity,
            0
          );
        }
      ...
      const cancellationResponse: OrderCancellationResponseDTO = {
        success: true,
        message:
          "Order refunded, tickets cancelled, and coins returned to user.",
        data: {
          orderId: order._id.toString(),
          status: "cancelled",
          cancelledAt: new Date(),
          // reason: can be added if your flow provides it (e.g., req.body.reason)
        },
      };

      return res.json(cancellationResponse);
    } catch (err: any) {
      return res.status(500).json({ success: false, message: err.message });
    }
  }
...
export const resendOrderTickets = async (
  ...
  try {
    ...
    const tickets = [];
    for (const ticket of order.tickets) {
      const ticketTier = await Event.findById(order.event)
        .select("ticketTiers")
        .then((event) =>
      ...
      const qrPayload =
        ...
        `Event: ${eventDoc?.name || ""}\n` +
        `Date: ${
          eventDoc?.date ? eventDoc.date.toISOString().split("T")[0] : ""
        }\n` +
        `Venue: ${eventDoc?.venue?.name || ""}\n` +
    ...
    return res
      .status(200)
      .json({ success: true, message: "Tickets resent successfully", tickets });
  } catch (err: any) {
    return res.status(500).json({ success: false, message: err.message });
  }
};
