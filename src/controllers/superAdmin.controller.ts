...
export class SuperAdminController {
  ...
  loginSuperAdmin = async (req: Request, res: Response) => {
    try {
      ...
      if (role !== "super_admin") {
        return res
          .status(400)
          .json({ success: false, message: "Invalid role for this endpoint" });
      }
  ...
  verifySuperAdminEmail = async (req: Request, res: Response) => {
    try {
      const { token } = req.query;
      const superAdmin = await this.superAdminService.verifyEmail(
        token as string
      );
      res.status(200).json({
        success: true,
  ...
  approveSuperAdmin = async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const superAdmin = await this.superAdminService.approveSuperAdmin(id);
      res.status(200).json({
        success: true,
        message: "Super admin approved",
        data: { superAdmin },
      });
    } catch (error: any) {
      res.status(400).json({ success: false, message: error.message });
  ...
  getAllSuperAdmins = async (req: Request, res: Response) => {
    try {
  ...
  getMyProfile = async (req, res) => {
    try {
      const superAdmin = await this.superAdminService.findById(req.user._id);
      if (!superAdmin)
        return res
          .status(404)
  ...
  updateMyProfile = async (req, res) => {
    try {
      ...
      const superAdmin = await this.superAdminService.updateById(
        req.user._id,
        updates
      );
      if (!superAdmin)
  ...
  deleteMyProfile = async (req, res) => {
    try {
      const superAdmin = await this.superAdminService.deleteById(req.user._id);
      if (!superAdmin)
        return res
          .status(404)
          .json({ success: false, message: "Profile not found" });
  ...
  disapproveSuperAdmin = async (req, res) => {
    try {
      const { id } = req.params;
      // Set isApproved to false for super admin
      const superAdmin = await this.superAdminService.updateById(id, {
        isApproved: false,
      });
      if (!superAdmin)
        return res
      ...
      for (const outlet of outlets) {
        ...
        outlet.set("wasActiveBeforeDeactivation", outlet.isActive, {
          strict: false,
        });
        ...
        if (outlet.assignedAdmin) {
          ...
          if (outletAdmin) {
            outletAdmin.set(
              "wasActiveBeforeDeactivation",
              outletAdmin.isActive,
              { strict: false }
            );
        ...
        await Offer.updateMany(
          { outlet: outlet._id },
          { $set: { isActive: false, wasActiveBeforeDeactivation: true } }
        );
      ...
      res.status(200).json({
        success: true,
        message:
          "Super admin disapproved and all related entities deactivated.",
      });
    } catch (error) {
      res.status(500).json({ success: false, message: error.message });
    }
  ...
  getDashboardData = async (req, res) => {
    try {
      ...
      const totalTxnAgg = await Payment.aggregate([
        {
          $match: {
            outletId: { $in: outletIds.map((id) => id.toString()) },
            type: "dine-in",
            status: "completed",
