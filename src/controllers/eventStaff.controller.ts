...
export class EventStaffController {
    ...
    try {
      ...
      if (user.role === "event_organizer") {
        ...
        return res.status(403).json({
          success: false,
          message: "Only event organizers or managers can create staff.",
        });
      }
  ...
  async assignEventStaffToEvent(req: Request, res: Response) {
    try {
      ...
      if (!eventId || !eventStaffId) {
        return res.status(400).json({
          success: false,
          message: "Missing required fields: eventId, eventStaffId",
        });
      }
      ...
      if (user.role === "event_organizer") {
        ...
        if (!hasPermission) {
          return res.status(403).json({
            success: false,
            message:
              "Forbidden: You can only assign staff to events you created.",
          });
        }
      } else if (user.role === "event_manager") {
        ...
        if (!hasPermission) {
          return res.status(403).json({
            success: false,
            message:
              "Forbidden: You can only assign staff to events you are assigned to manage.",
          });
        }
      } else {
        return res.status(403).json({
          success: false,
          message:
            "Forbidden: Only event organizers and event managers can assign staff to events.",
        });
      }
      ...
      const getEventRange = (ev: any) => {
        ...
        if (hasMultiDay) {
          ...
          if (
            isNaN(startEventDate.getTime()) ||
            isNaN(endEventDate.getTime())
          ) {
          ...
          const end = new Date(endEventDate);
            ...
            Number.isFinite(eh) ? eh : 23,
            Number.isFinite(em) ? em : 59,
            59,
            999
          );
          return { start, end };
      ...
      if (!newStart || !newEnd) {
        return res.status(400).json({
          success: false,
          message: "Event date/time not configured for the selected event",
        });
      }
      ...
      if (
        (staff as any).assignedEvents &&
        Array.isArray((staff as any).assignedEvents)
      ) {
        ...
        for (const ev of existingEvents) {
          ...
          if (!start || !end) {
            ...
            if (sameDay) {
              return res.status(409).json({
                success: false,
                message:
                  "Event staff is already assigned to another event on the same date and time.",
              });
            }
            continue;
          }
          if (isOverlap(start, end, newStart, newEnd)) {
            return res.status(409).json({
              success: false,
              message:
                "Event staff is already assigned to another event during this time window.",
            });
          }
        }
      }
      ...
      const currentAssigned: string[] = Array.isArray(
        (staff as any).assignedEvents
      )
        ? (staff as any).assignedEvents.map((id: any) => id.toString())
        : [];
  ...
  async updateProfile(req: Request, res: Response) {
    try {
      ...
      const staff = await EventStaff.findByIdAndUpdate(user._id, updates, {
        new: true,
      });
      if (!staff) {
        return res
  ...
  async getDashboardData(
    ...
    try {
      ...
      const totalTicketsChecked = await Ticket.countDocuments({
        scannedBy: staffId,
      });
  ...
  async getAssignedEventTicketBookings(
    req: Request & { user?: { _id: string; role: string } },
    res: Response
  ) {
    try {
      ...
      if (!isAssignedToEvent) {
        return res.status(403).json({
          success: false,
          message:
            "Forbidden: You can only access ticket bookings for events you are assigned to",
        });
      }
