...
export class EventController {
  ...
  async createEvent(req: Request & { user?: { _id: string } }, res: Response) {
    try {
      ...
      if (
        Array.isArray(req.body.ticketTiers) &&
        req.body.venue &&
        req.body.venue.capacity
      ) {
        const totalSeats = req.body.ticketTiers.reduce(
          (sum: number, tier: any) => sum + (Number(tier.quantity) || 0),
          0
        );
        if (totalSeats > req.body.venue.capacity) {
          return res.status(400).json({
            success: false,
            message: `Total ticket tier seats (${totalSeats}) exceed venue capacity (${req.body.venue.capacity})`,
          });
        }
      }
      ...
      const validateSaleDates = () => {
        ...
        if (req.body.date) {
          ...
          if (!endTime) {
            return {
              valid: false,
              message: "End time is required for event creation",
            };
          }
          ...
          return {
            valid: false,
            message:
              "Either date (for single-day events) or startEventDate and endEventDate (for multi-day events) are required",
          };
        }
        ...
        if (req.body.saleStart) {
          ...
          if (saleStart >= eventEndingDateTime) {
            return {
              valid: false,
              message:
                "Sale start date and time must be before the event ending date and time",
            };
          }
        }
        ...
        if (req.body.saleEnd) {
          ...
          if (saleEnd >= eventEndingDateTime) {
            return {
              valid: false,
              message:
                "Sale end date and time must be before the event ending date and time",
            };
          }
        }
        ...
        if (req.body.saleStart && req.body.saleEnd) {
          ...
          if (saleStart >= saleEnd) {
            return {
              valid: false,
              message:
                "Sale start date and time must be before sale end date and time",
            };
          }
        }
      ...
      if (!saleDateValidation.valid) {
        return res
          .status(400)
          .json({ success: false, message: saleDateValidation.message });
      }
      ...
      return res
        .status(201)
        .json({ success: true, data: formatNamesCamelCase(event) });
    } catch (err: any) {
    ...
    try {
      ...
      if (managerId) {
        ...
        if (!mgrName || !email || !password || !phone) {
          return res.status(400).json({
            success: false,
            message: "All manager fields are required",
          });
        }
        if (!isValidEmail(email)) {
          return res
            .status(400)
            .json({ success: false, message: "Invalid manager email format" });
        }
        if (!isValidPhone(phone)) {
          return res
            .status(400)
            .json({ success: false, message: "Invalid manager phone format" });
        }
        ...
        if (existingPhone) {
          return res.status(409).json({
            success: false,
            message: "Manager phone number already exists",
          });
        }
        ...
        return res
          .status(400)
          .json({ success: false, message: "Invalid manager format" });
      }
      // If manager assignment is expected but managerId is missing, throw error
      if (manager && !managerIdToUse) {
        return res.status(400).json({
          success: false,
          message: "Manager assignment expected but managerId is missing.",
        });
      }
  ...
  async updateDraft(req: Request & { user?: { _id: string } }, res: Response) {
    try {
      ...
      if (!event) {
        return res
          .status(404)
          .json({ success: false, message: "Draft event not found" });
      }
      // Only allow update if user is creator or assigned manager
      if (
        event.createdBy.toString() !== userId &&
        (!event.managerId || event.managerId.toString() !== userId)
      ) {
        return res.status(403).json({
          success: false,
          message: "Forbidden: Not allowed to update this event",
        });
      }

      // Validation: ticketPrice logic based on ticketTiers (if updating these fields)
      if (
        req.body.ticketTiers !== undefined ||
      ...
      if (req.body.startEventDate && req.body.endEventDate) {
        ...
        if (startEventDate > endEventDate) {
          return res.status(400).json({
            success: false,
            message: "Start event date cannot be after end event date.",
          });
        }
      ...
      const validateSaleDates = () => {
        ...
        if (!eventEndingDateTime) {
          return {
            valid: false,
            message: "Unable to determine event ending date and time",
          };
        }
    ...
    try {
      ...
      if (files && Array.isArray(files)) {
        if (files.length < 1 || files.length > 3) {
          return res.status(400).json({
            success: false,
            message: "You must upload between 1 and 3 cover images.",
          });
        }
  ...
  async publishEvent(req: Request & { user?: { _id: string } }, res: Response) {
    try {
      ...
      if (
        ...
        return res.status(403).json({
          success: false,
          message: "Forbidden: Not allowed to publish this event",
        });
      }
      ...
      if (!hasSingleDate && !hasMultiDayDates) {
        return res.status(400).json({
          success: false,
          message:
            "Missing required field: date (for single-day events) or startEventDate and endEventDate (for multi-day events)",
        });
      }
      ...
      if (!hasTicketTiers) {
        ...
        if (event.ticketPrice === undefined || event.ticketPrice === null) {
          return res.status(400).json({
            success: false,
            message:
              "ticketPrice is required when publishing an event without ticket tiers",
          });
        }
        if (typeof event.ticketPrice !== "number" || event.ticketPrice < 0) {
          return res.status(400).json({
            success: false,
            message:
              "ticketPrice must be a non-negative number when publishing an event without ticket tiers",
          });
        }
      }
      ...
      let totalTierQty = 0;
      if (hasTicketTiers) {
        totalTierQty = event.ticketTiers.reduce(
      ...
      if (capacity > 0 && totalTierQty > capacity) {
        return res.status(400).json({
          success: false,
          message: `Total ticket tier seats (${totalTierQty}) exceed venue capacity (${capacity})`,
        });
      }
      ...
      return res
        .status(200)
        .json({ success: true, data: formatNamesCamelCase(serialized) });
    } catch (err: any) {
      return res.status(400).json({ success: false, message: err.message });
    }
  }
  ...
  async getMyEvents(
    ...
    try {
      ...
      if (!user || user.role !== "event_organizer") {
        return res.status(403).json({
          success: false,
          message: "Only event organizers can access their events.",
        });
      }
  ...
  async getMyManagedEvents(
    ...
    try {
      ...
      if (!user || user.role !== "event_manager") {
        return res.status(403).json({
          success: false,
          message: "Only event managers can access their assigned events.",
        });
      }
      ...
      return res
        .status(200)
        .json({ success: true, data: eventsWithCancellationInfo });
    } catch (err: any) {
      return res.status(500).json({ success: false, message: err.message });
    }
  }

  async getMyStaffEvents(
    req: Request & { user?: { _id: string; role: string } },
    res: Response
  ) {
    try {
      ...
      if (!user || user.role !== "event_staff") {
        return res.status(403).json({
          success: false,
          message: "Only event staff can access their assigned events.",
        });
      }
      ...
      if (!staffRecord) {
        return res
          .status(404)
          .json({ success: false, message: "Event staff record not found." });
      }
      ...
      if (
        staffRecord.assignedEvents &&
        Array.isArray(staffRecord.assignedEvents)
      ) {
        ...
        if (event.date) {
          ...
          if (eventDate.getTime() === today.getTime()) {
            ...
            if (event.startTime && event.endTime) {
              ...
              if (
                currentTime >= eventStartMinutes &&
                currentTime <= eventEndMinutes
              ) {
    ...
    try {
      ...
      if (
        ...
        return res.status(403).json({
          success: false,
          message:
            "Only event organizers or managers can access their event staff.",
        });
      }
      ...
      const eventStaff = await EventStaff.find({
  ...
  async activateEventStaff(
    ...
    try {
      ...
      if (!user || !["event_organizer", "event_manager"].includes(user.role)) {
        return res.status(403).json({
          success: false,
          message: "Only event organizer or manager can activate staff.",
        });
      }
      ...
      if (!staff) {
        return res
          .status(404)
          .json({ success: false, message: "Event staff not found." });
      }
      if (staff.isActive) {
        return res.status(400).json({
          success: false,
          message: "Event staff is already activated.",
        });
      }
      ...
      if (staff.event) {
        ...
        if (event) {
          ...
          allow =
            user.role === "event_organizer" || user.role === "event_manager";
        }
      } else {
        allow =
          user.role === "event_organizer" || user.role === "event_manager";
      ...
      if (!allow) {
        return res.status(403).json({
          success: false,
          message: "Forbidden: Not allowed to activate staff.",
        });
      }
      staff.isActive = true;
  ...
  async deactivateEventStaff(
    ...
    try {
      ...
      if (!user || !["event_organizer", "event_manager"].includes(user.role)) {
        return res.status(403).json({
          success: false,
          message: "Only event organizer or manager can deactivate staff.",
        });
      }
      const staff = await EventStaff.findById(staffId);
      if (!staff) {
      ...
      if (!staff.isActive) {
        return res.status(400).json({
          success: false,
          message: "Event staff is already deactivated.",
        });
      }
      // Permission check: if event is assigned, check creator/manager; else allow both event_organizer and event_manager
      let allow = false;
      ...
      if (!allow) {
        return res.status(403).json({
          success: false,
          message: "Forbidden: Not allowed to deactivate staff.",
        });
      }
    ...
    try {
      ...
      if (files && Array.isArray(files) && files.length > 0) {
        ...
        const uploadPromises = files.map(async (file: any) => {
          const b64 = Buffer.from(file.buffer).toString("base64");
          const dataURI = `data:${file.mimetype};base64,${b64}`;
          const result = await import("../config/cloudinary").then((mod) =>
            mod.default.uploader.upload(dataURI, {
              folder: "event-covers",
              resource_type: "auto",
            })
          );
      ...
      if (Array.isArray(req.body.ticketTiers)) {
        ...
        if (capacity > 0 && totalSeats > capacity) {
          return res.status(400).json({
            success: false,
            message: `Total ticket tier seats (${totalSeats}) exceed venue capacity (${capacity})`,
          });
        }
  ...
  async getEventById(req: Request, res: Response) {
    try {
      ...
      if (
        eventTicketTiers &&
        Array.isArray(eventTicketTiers) &&
        eventTicketTiers.length > 0
      ) {
        totalSoldCount = eventTicketTiers.reduce(
          (sum, tier) => sum + (tier.soldCount || 0),
          0
        );
      } else {
      ...
      if (
        ...
        totalSeats = eventTicketTiers.reduce(
        ...
        availableSeats =
          event.venue.capacity - totalSoldCount - reservedQuantity;
      }
      ...
      if (eventTicketTiers && Array.isArray(eventTicketTiers)) {
        ticketTiersWithAvailable = eventTicketTiers.map((tier) => {
          ...
          const activeSessionsForTier = Array.from(
            ...
            (session) =>
              session.tierId === tier._id?.toString() && session.eventId === id
          );

          const reservedForTier = activeSessionsForTier.reduce(
            (sum, session) => sum + session.quantity,
            0
          );
          const actuallyAvailable =
            (tier.quantity || 0) - (tier.soldCount || 0) - reservedForTier;
  ...
  async listEvents(req: Request, res: Response) {
    try {
      const { search, date, location, category, minPrice, maxPrice, upcoming } =
        req.query;
      ...
      const events = await Event.find(filter).select(
        "name date startEventDate endEventDate venue coverImages type ticketPrice ticketTiers startTime endTime isCancelled cancelledBy cancelledAt cancellationDescription cancellationInstructions"
      );
      ...
      const eventsWithType = events.map((event) => {
        ...
        // Handle multi-day events
          ...
          if (
            today.getTime() >= startEventDate.getTime() &&
            today.getTime() <= endEventDate.getTime()
          ) {
  ...
  async getDashboardData(
    ...
    try {
      ...
      const eventManagerCount = await EventManager.countDocuments({
        createdBy: organizerId,
      });
      // 4. Event staff count
  ...
  async getEventTicketBookings(
    ...
    try {
      ...
      if (user.role === "event_organizer") {
        ...
        hasPermission =
          Boolean(event.managerId) &&
          event.managerId!.toString() === user._id.toString();
        ...
        if (
          ...
          staff.assignedEvents.includes(eventId as any)
        ) {
          hasPermission = true;
        }
      }
      ...
      if (!hasPermission) {
        return res.status(403).json({
          success: false,
          message:
            "Forbidden: You do not have permission to access this event's ticket bookings",
        });
      }
  ...
  async cancelEvent(
    ...
    try {
      ...
      if (
        !["event_organizer", "event_manager", "cityfeed_admin"].includes(
          user.role
        )
      ) {
        return res.status(403).json({
          success: false,
          message:
            "Forbidden: Only event organizers and event managers can cancel events",
        });
      }
      ...
      if (
        ...
        event.createdBy.toString() !== user._id
      ) {
        return res.status(403).json({
          success: false,
      ...
      if (user.role === "event_manager") {
        ...
        if (!isAssignedManager) {
          return res.status(403).json({
            success: false,
            message:
              "Forbidden: Only the event creator or assigned manager can cancel this event",
          });
        }
      }
      ...
      if (!otp) {
        return res.status(400).json({
          success: false,
          message:
            "OTP is required for event cancellation. Please request an OTP first.",
          data: {
            requiresOTP: true,
            message:
              "Use /api/events/{eventId}/cancel/request-otp to get verification code",
          },
        });
      }
      ...
      try {
        ...
        const otpService = OTPService.getInstance(this.sendGridService);

        // Get user's email for OTP verification
        const { User } = await import("../models/user.model");
        const { EventOrganizer } = await import(
          "../models/eventOrganizer.model"
        );
        ...
        if (!userDoc || !userDoc.email) {
          return res.status(400).json({
            success: false,
            message:
              "Email address required for event cancellation. Please add an email address to your profile.",
            data: {
              requiresEmail: true,
              message:
                "Event organizers and managers must have a valid email address for security verification",
              instructions:
                "Please update your profile with a valid email address and try again",
            },
          });
        }

        const isOTPValid = await otpService.verifyEmailOTP(
          userDoc.email,
          otp,
          "event_cancellation"
        );
      ...
      if (tickets.length > 0) {
        ...
        const emailPromises = tickets.map(async (ticket) => {
          try {
            ...
            console.error(
              `Failed to send cancellation email to user ${ticket.userId}:`,
              error
            );
          }
        });
        ...
        Promise.allSettled(emailPromises).then((results) => {
          ...
          logger.info(
            `Event cancellation emails: ${successful} sent, ${failed} failed`
          );
        });
      }
  ...
  async requestCancellationOTP(
    ...
    try {
      ...
      logger.info(
        "✅ [DEBUG] User has permission to cancel this specific event",
        {
          userId: user._id,
          userRole: user.role,
          eventId: event._id,
      ...
      logger.info(
        "✅ [DEBUG] Event is not cancelled, proceeding with OTP request"
      );
      ...
      try {
        ...
        logger.info("🔍 [DEBUG] OTPService singleton instance retrieved");

        const otpResult = await otpService.sendOTPToEmail(
          userDoc.email,
          "event_cancellation"
        );
        ...
        return res.status(200).json({
          ...
          data: {
            ...
            email: userDoc.email,
            message:
              "Please check your email and enter the OTP to confirm event cancellation",
            expiresIn: "5 minutes",
          },
        });
      } catch (otpError: any) {
    ...
    try {
      ...
      if (!["event_organizer", "event_manager"].includes(userRole || "")) {
        return res.status(403).json({
          success: false,
          message:
            "Forbidden: Only event organizers and event managers can view ticket holders",
        });
      }
      ...
      if (!isCreator && !isManager) {
        return res.status(403).json({
          success: false,
          message:
            "Forbidden: Only the event creator or assigned manager can view ticket holders for this event",
        });
      }
