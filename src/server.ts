...
const activeBookingSessions = new Map<string, {
  userId: string;
  eventId: string;
  tierId: string;
  quantity: number;
...
io.on('connection', (socket) => {
  ...
  socket.on('startBooking', async (data: {
    eventId: string;
    userId: string;
    tierId: string;
    quantity: number;
    ...
    try {
      ...
      const activeSessionsForTier = Array.from(activeBookingSessions.values())
        .filter(session => session.tierId === tierId && session.eventId === eventId);
      ...
      socket.emit('bookingError', { message: 'Failed to start booking session' });
    }
  });
  ...
  socket.on('cancelBooking', (data: { sessionId: string; userId: string }) => {
    ...
    if (session && session.userId === userId) {
      ...
      socket.emit('bookingCancelled', { sessionId, message: 'Booking session cancelled' });
      logger.info(`Booking session cancelled: ${sessionId}`);
    }
  });

  // Complete booking (called after successful payment)
  socket.on('completeBooking', (data: { sessionId: string; userId: string }) => {
    const { sessionId, userId } = data;
    const session = activeBookingSessions.get(sessionId);
    
    if (session && session.userId === userId) {
      activeBookingSessions.delete(sessionId);
      socket.emit('bookingCompleted', { sessionId, message: 'Booking completed successfully' });
  ...
  socket.on('getAvailability', async (data: { eventId: string }) => {
    try {
      ...
      socket.emit('availabilityError', { message: 'Failed to get availability data' });
    }
  });
