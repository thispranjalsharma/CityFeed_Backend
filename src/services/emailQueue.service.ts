...
interface EmailJob {
  id: string;
  type: 'verification' | 'passwordReset' | 'ticket' | 'dineInSummary' | 'adminNotification';
  data: any;
  retries: number;
  maxRetries: number;
  createdAt: Date;
...
export class EmailQueueService {
  ...
  async addToQueue(type: EmailJob['type'], data: any): Promise<void> {
    try {
      if (!type || !data) {
        logger.error('Invalid parameters for email queue:', { type, hasData: !!data });
  ...
  private async processQueue(): Promise<void> {
    ...
    logger.info(`Starting email queue processing. Queue length: ${this.queue.length}`);
      ...
      while (this.queue.length > 0) {
        ...
        try {
          ...
          if (job.retries < job.maxRetries) {
            job.retries++;
            logger.info(`Retrying email job: ${job.id} (attempt ${job.retries}/${job.maxRetries})`);
            ...
            setTimeout(() => {
              try {
                ...
                if (!this.isProcessing) {
                  this.processQueue().catch(error => {
                    logger.error('Error in email queue processing during retry:', error);
                  });
                }
              } catch (error) {
            ...
            logger.error(`Email job failed permanently after ${job.maxRetries} retries: ${job.id}`);
          }
        }
  ...
  async sendTicketEmail(data: any): Promise<void> {
    try {
      if (!data || !data.to || !data.event || !data.tickets || !data.userName) {
        logger.error('Missing required parameters for ticket email:', { hasData: !!data, hasTo: !!data?.to, hasEvent: !!data?.event, hasTickets: !!data?.tickets, hasUserName: !!data?.userName });
  ...
  async sendDineInSummaryEmail(data: any): Promise<void> {
    try {
      if (!data || !data.to || !data.userName || !data.outletName || !data.reviewLink) {
        logger.error('Missing required parameters for dine-in summary email:', { hasData: !!data, hasTo: !!data?.to, hasUserName: !!data?.userName, hasOutletName: !!data?.outletName, hasReviewLink: !!data?.reviewLink });
  ...
  async sendAdminNotification(superAdmin: any): Promise<void> {
    try {
      if (!superAdmin || !superAdmin.name || !superAdmin.email || !superAdmin.phone) {
        logger.error('Missing required parameters for admin notification:', { hasSuperAdmin: !!superAdmin, hasName: !!superAdmin?.name, hasEmail: !!superAdmin?.email, hasPhone: !!superAdmin?.phone });
  ...
  getQueueStatus(): { length: number; isProcessing: boolean; oldestJob?: Date } {
    try {
      const oldestJob = this.queue.length > 0 ? this.queue[0].createdAt : undefined;
      return {
