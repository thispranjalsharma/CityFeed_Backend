...
export interface ICronJobService {
  cleanupOldSoftDeletedRecords(): Promise<void>;
  getSoftDeleteStatistics(): Promise<any>;
}
export class CronJobService implements ICronJobService {
  async cleanupOldSoftDeletedRecords(): Promise<void> {
    try {
      ...
      logger.info(
        `Cleaned up ${cleanupResults.offers} old soft-deleted offers`
      );
      ...
      logger.info(
        `Cleaned up ${cleanupResults.outlets} old soft-deleted outlets`
      );
      ...
      logger.info(
        `Cleaned up ${cleanupResults.employees} old soft-deleted employee assignments`
      );
      ...
      logger.info(
        `Cleaned up ${cleanupResults.outletAdmins} old soft-deleted outlet admins`
      );

      cleanupResults.total =
        cleanupResults.offers +
        cleanupResults.outlets +
        cleanupResults.employees +
        cleanupResults.users +
        cleanupResults.outletAdmins;
  ...
  async getSoftDeleteStatistics(): Promise<any> {
    try {
      ...
      stats.offers.totalSoftDeleted = await Offer.countDocuments({
        isDeleted: true,
      });
      ...
      stats.outlets.totalSoftDeleted = await Outlet.countDocuments({
        isDeleted: true,
      });
      ...
      stats.employees.totalSoftDeleted = await Staff.countDocuments({
        isDeleted: true,
      });
      ...
      stats.users.totalSoftDeleted = await User.countDocuments({
        isDeleted: true,
      });
      ...
      stats.outletAdmins.totalSoftDeleted = await OutletAdmin.countDocuments({
        isDeleted: true,
      });
