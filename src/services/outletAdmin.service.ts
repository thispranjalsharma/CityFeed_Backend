import { IOutletAdmin, OutletAdmin } from "../models/outletAdmin.model";
...
export interface IOutletAdminService {
  createOutletAdmin(data: Partial<IOutletAdmin>): Promise<IOutletAdmin>;
  findByEmail(email: string): Promise<IOutletAdmin | null>;
  findById(id: string): Promise<IOutletAdmin | null>;

  // deleteOutletAdmin(id: string): Promise<IOutletAdmin | null>;
  login(
    email: string,
    password: string
  ): Promise<{
    outletAdmin: IOutletAdmin;
    token: string;
    outletId: string | null;
  }>;
  sendVerificationEmail(outletAdmin: IOutletAdmin): Promise<void>;
  verifyEmail(token: string): Promise<IOutletAdmin>;
  updatePassword(id: string, newPassword: string): Promise<IOutletAdmin>;
  changePassword(
    id: string,
    currentPassword: string,
    newPassword: string
  ): Promise<IOutletAdmin>;
  activateAccount(id: string): Promise<IOutletAdmin>;
  deactivateAccount(id: string): Promise<IOutletAdmin>;
  // Soft delete outlet admin
  softDeleteOutletAdmin(id: string): Promise<IOutletAdmin | null>;
  // Restore soft deleted outlet admin
  restoreOutletAdmin(id: string): Promise<IOutletAdmin | null>;
  // Get deleted outlet admins
  getDeletedOutletAdmins(): Promise<IOutletAdmin[]>;
}

@injectable()
export class OutletAdminService implements IOutletAdminService {
  constructor(
  ...
  async createOutletAdmin(data: Partial<IOutletAdmin>): Promise<IOutletAdmin> {
    ...
    if (existing)
      throw new Error("Outlet admin already exists with this email");

    // Check if phone number is already registered
    if (data.phone) {
      const existingByPhone = await this.outletAdminRepository.findByPhone(
        data.phone
      );
  ...
  async sendVerificationEmail(outletAdmin: IOutletAdmin) {
    const token = jwt.sign({ _id: outletAdmin._id }, config.jwtSecret, {
      expiresIn: "7d",
    });
