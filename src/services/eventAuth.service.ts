import {
  EventOrganizer,
  IEventOrganizer,
} from "../models/eventOrganizer.model";
...
export interface IEventAuthService {
  registerEventOrganizer(data: {
    name: string;
    email: string;
    password: string;
    phone: string;
  }): Promise<{ organizer: IEventOrganizer; token: string }>;
  verifyEmail(token: string): Promise<any>;
  resendVerification(email: string): Promise<void>;
  resendManagerVerification(email: string): Promise<void>;
  resendStaffVerification(email: string): Promise<void>;
  ...
  sendOrganizerPasswordResetEmail(
    email: string
  ): Promise<{ message: string; token: string }>;
  validatePasswordStrength(password: string): void;
  resetManagerPassword(token: string, password: string);
  resetStaffPassword(token: string, password: string);
  changeOrganizerPassword(
    organizerId: string,
    currentPassword: string,
    newPassword: string
  ): Promise<IEventOrganizer>;
  resetOrganizerPassword(
    token: string,
    password: string
  ): Promise<IEventOrganizer>;
  generateAndSendManagerVerification(manager: IEventManager): Promise<string>;
}

@injectable()
export class EventAuthService implements IEventAuthService {
  constructor(
    @inject("SendGridService") private sendGridService: SendGridService
  ) {}

  async registerEventOrganizer(data: {
    name: string;
    email: string;
    password: string;
    phone: string;
    ...
    const existingByPhone = await EventOrganizer.findOne({
      phone: data.phone,
      isDeleted: false,
    });
    if (existingByPhone)
  ...
  async resendVerification(email: string): Promise<void> {
    ...
    if (organizer.isEmailVerified)
      throw new AppErrorClass("Email is already verified", 400);
    const token = generateToken({
      _id: organizer._id.toString(),
      email: organizer.email,
      role: "event_organizer",
  ...
  async generateAndSendManagerVerification(
